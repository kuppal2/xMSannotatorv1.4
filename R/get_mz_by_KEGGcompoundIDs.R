...
function(keggIDs,queryadductlist=c("M+H"),syssleep=0.01,adduct_table=NA){
  ...
  if(isTRUE((is.na(adduct_table)==TRUE)[1]))
  {
    rm(adduct_table)
    data(adduct_table)
  ...
  if(isTRUE((queryadductlist[1]=="positive")[1]))
  {
    ...
    if(isTRUE((queryadductlist[1]=="negative")[1]))
    {
      ...
      if(isTRUE((queryadductlist[1]=="all")[1])){
        ...
        if(isTRUE((length(which(queryadductlist%in%alladducts==FALSE))>0)[1])){
  ...
  for(i in seq_along(keggIDs)){
    ...
    if(isTRUE((is(d2,"try-error"))[1])){
      ...
      if(isTRUE((length(exact_mass)>0)[1])){
